...
from verl.trainer.ppo.mix_trainer import MixRayPPOTrainer
...
class TaskRunner:
    ...
    def add_actor_rollout_worker(self, config):
        ...
        if config.actor_rollout_ref.actor.strategy in {"fsdp", "fsdp2"}:
            ...
            from verl.workers.mix_fsdp_workers import MIXActorRolloutRefWorker, AsyncActorRolloutRefWorker


            actor_rollout_cls = (
                AsyncActorRolloutRefWorker
                if config.actor_rollout_ref.rollout.mode == "async"
                else MIXActorRolloutRefWorker
            )
        ...
        from verl.trainer.ppo.mix_trainer import Role
        # 调试输出
    ...
    def run(self, config):
        ...
        train_dataset = create_rl_train_dataset_with_targets(config.data.train_files, config.data, tokenizer, processor, is_train=True)
        val_dataset = create_rl_dataset(config.data.val_files, config.data, tokenizer, processor, is_train=False)
        train_sampler = create_rl_sampler(config.data, train_dataset)

        # Initialize the Mix PPO trainer.
        from verl.trainer.ppo.mix_trainer import MixRayPPOTrainer
        # print('Here')

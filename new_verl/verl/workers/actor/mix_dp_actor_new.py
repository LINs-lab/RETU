...
from verl.workers.actor.mix_actor_friends import pack_vanilla_rl_loss_inputs, seperate_on_off, get_pure_sft_loss, rl_loss_fn_router, get_policy_loss, append_ppo_actor_raw_metrics
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    if self.config.offline_loss_type == "sft" or use_sft: # UPT case
                        print('offline_loss_type sft is used')
                        # 整理 loss_fn 的输入
                        vanilla_elements = pack_vanilla_rl_loss_inputs(old_log_prob, log_prob, advantages, response_mask, loss_agg_mode, self.config)
                        off_elements, on_elements = seperate_on_off(model_inputs, log_prob, old_log_prob, response_mask, advantages)
                        sft_loss = get_pure_sft_loss(off_elements)   
                        loss_mode = self.config.policy_loss.get("loss_mode", "vanilla")
                        policy_loss_fn = rl_loss_fn_router(loss_mode)
                        # 此时用vanilla pg loss, 不区分 on & off_policy loss
                        pg_loss, metrics = get_policy_loss(policy_loss_fn, vanilla_elements, on_elements=None, metrics=metrics)
                    ...
                    elif self.config.offline_loss_type == "off_policy": # LUFFY case
                        vanilla_elements = pack_vanilla_rl_loss_inputs(old_log_prob, log_prob, advantages, response_mask, loss_agg_mode, self.config)
                        off_elements, on_elements = seperate_on_off(model_inputs, log_prob, old_log_prob, response_mask, advantages)
                        # 补充 target_probs， LUFFY policy loss 输入必备
                        target_probs = model_inputs['target_probs'] if 'target_probs' in model_inputs else None
                        on_elements['target_probs'] = target_probs
                        # luffy 没有额外的 sft loss
                        loss_mode = self.config.offline_loss_type
                        policy_loss_fn = rl_loss_fn_router(loss_mode)
                        # LUFFY 算pg_loss 需要区分 on/ off policy data
                        pg_loss, metrics = get_policy_loss(policy_loss_fn, vanilla_elements, on_elements=on_elements, metrics=metrics)
                        ...
                        off_elements['off_policy_logprob'] = (log_prob * (sft_coef.view(-1, 1)))[off_elements['off_policy_mask']]
                        sft_loss = get_pure_sft_loss(off_elements) 
                        ...
                        on_elements.update(srft_elements)
                        loss_mode = self.config.offline_loss_type
                        # SRFT 算pg_loss 需要区分 on/ off policy data
                        policy_loss_fn = rl_loss_fn_router(loss_mode)
                        pg_loss, metrics = get_policy_loss(policy_loss_fn, vanilla_elements, on_elements=on_elements, metrics=metrics)
                        srft_metric_data = {
                        ...
                        append_to_dict(metrics, srft_metric_data)
                        if torch.isnan(sft_loss):
                            print('sft_loss is nan, skipping sft_loss')
                        else:

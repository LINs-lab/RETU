...
from verl.workers.rollout.vllm_rollout.mix_vllm_rollout import MIXvLLMRollout
...
class MIXActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        print(f"=== _build_rollout Debug ===")
        print(f"Original config.rollout.name: {self.config.rollout.name}")
        print(f"rollout_config.name: {rollout_config.name}")
        print(f"rollout_config type: {type(rollout_config)}")
        ...
        print(f"rollout_worker.rollout type: {type(rollout_worker.rollout)}")
        print(f"rollout_worker.rollout type: {type(rollout_worker.rollout)}")
        print(f"rollout_worker.rollout has generate_on_sequences: {hasattr(rollout_worker.rollout, 'generate_on_sequences')}")
        if "vllm" in rollout_name:
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            # )
            from verl.workers.actor import MixDataParallelPPOActor
        ...
        if self._is_rollout:
            ...
            # if self.config.rollout.name == 'mix_vllm':
            #     self.rollout = self.rollout.rollout
            # else:
            #     pass

        print(f"=== After _build_rollout call ===")
        print(f"Received rollout_worker type: {type(self.rollout)}")
        # print(f"Received rollout_worker.rollout type: {type(self.rollout.rollout)}")
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="rollout"))
    @DistProfiler.annotate(color="crimson", role="generate_off_sequences")
    def generate_off_sequences(self, prompts: DataProto):
        ...
        with self.rollout_sharding_manager:
            ...
            with simple_timer("generate_off_sequences", timing_generate):
        ...
        # to make sure meta_info["timing"] is the same
        timing_generate_topk_ratio, timing_generate_min, timing_generate_max = topk_reduce_ratio_min_max(
            timing_generate["generate_off_sequences"]
        )
        timing_generate = reduce_timing(timing_generate)
        timing_generate.update(
            {
                "generation_off_timing/max": timing_generate_max,
                "generation_off_timing/min": timing_generate_min,
                "generation_off_timing/topk_ratio": timing_generate_topk_ratio,
            }
        )
        output.meta_info["off_timing"] = timing_generate
        output = output.to("cpu")
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="rollout"))
    @DistProfiler.annotate(color="magenta", role="generate_on_sequences")
    def generate_on_sequences(self, prompts: DataProto):

        # 添加调试信息
        print(f"=== Debug self.rollout ===")
        print(f"self.rollout type: {type(self.rollout)}")
        print(f"self.rollout class name: {self.rollout.__class__.__name__}")
        print(f"self.rollout module: {self.rollout.__class__.__module__}")
        print(f"Is MIXvLLMRollout: {isinstance(self.rollout, MIXvLLMRollout)}")
        
        # 检查可用方法
        methods = [method for method in dir(self.rollout) if not method.startswith('_')]
        ...
        with self.rollout_sharding_manager:
            ...
            with simple_timer("generate_on_sequences", timing_generate):
                output = self.rollout.generate_on_sequences(prompts=prompts)
        ...
        # to make sure meta_info["timing"] is the same
        timing_generate_topk_ratio, timing_generate_min, timing_generate_max = topk_reduce_ratio_min_max(
            timing_generate["generate_on_sequences"]
        )
        timing_generate = reduce_timing(timing_generate)
        timing_generate.update(
            {
                "generation_on_timing/max": timing_generate_max,
                "generation_on_timing/min": timing_generate_min,
                "generation_on_timing/topk_ratio": timing_generate_topk_ratio,
            }
        )
        output.meta_info["on_timing"] = timing_generate
        output = output.to("cpu")
...
class AsyncActorRolloutRefWorker(MIXActorRolloutRefWorker):
    def _build_rollout(self, trust_remote_code=False):

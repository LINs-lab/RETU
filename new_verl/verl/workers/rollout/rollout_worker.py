...
class RolloutWorker(Worker):
    def __init__(self, config: RolloutConfig, model_config: HFModelConfig) -> None:
        ...
        if "vllm" in self.config.name:
            ...
            if  self.config.name == 'mix_vllm':
                from verl.workers.rollout.vllm_rollout import MIXvLLMRollout
                vllm_rollout_cls = MIXvLLMRollout
            else:
                pass
            print(f"Final vllm_rollout_cls: {vllm_rollout_cls}")

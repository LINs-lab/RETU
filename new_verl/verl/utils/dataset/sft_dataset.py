...
class SFTDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        def check_dict(df_keys):
            if ('reward_model' in df_keys) and ('extra_info' in df_keys):
                return True
            else:
                return False
        ...
        for parquet_file in self.parquet_files:
            ...
            # check dataframe
            if check_dict(list(dataframe.keys())):
                sample_row = dataframe.iloc[0]
                reward_model_sample = sample_row['reward_model']
                extra_info_sample =  sample_row['extra_info']
                if isinstance(reward_model_sample, str):
                    dataframe['reward_model'] = dataframe['reward_model'].apply(json.loads)
                if isinstance(extra_info_sample, str):
                    dataframe['extra_info'] = dataframe['extra_info'].apply(json.loads)
                print('convert reward_model, extra_info type from string to dict')
